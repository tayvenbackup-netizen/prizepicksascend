...
function Row({ label, badge }: { label: string; badge?: string }) {
  return (
    ...
    >
      ...
      <ChevronRight className="h-[18px] w-[18px] text-white/40" strokeWidth={1.75} />
    </button>
  );
}
      ...
      {open && (
        ...
        >
          ...
          <div className="px-5 pt-1">
            {/* Balance card */}
            <section className="rounded-2xl border border-white/[0.09] bg-[#0a0d18] p-4">
              <div className="flex items-start justify-between">
                <div>
                  <p className="text-[13px] text-white/85">PrizePicks Balance</p>
                  <p className="mt-1.5 text-[28px] font-extrabold leading-none tracking-tight text-white">{balance}</p>
                </div>
                <button aria-label="info" className="text-white/85">
                  <Info className="h-[19px] w-[19px]" strokeWidth={1.75} />
                </button>
              </div>

              <div className="mt-4 rounded-xl bg-[#171a26] px-3.5 py-3.5">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[11.5px] text-white/55 leading-tight">For player stats and performance</p>
                    </div>
                  </div>
                  <p className="text-[14px] text-white">{balance}</p>
                </div>
                <div className="mt-3 flex items-center justify-between pl-3">
                  <div className="flex items-center gap-2.5">
                    <svg viewBox="0 0 24 24" className="h-[14px] w-[14px] text-white/45" fill="none" stroke="currentColor" strokeWidth={1.75}>
                    ...
                    <div>
                      ...
                      <p className="mt-1 text-[11.5px] text-white/55 leading-tight">For games and trending topics</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-[14px] text-white leading-tight">{balance}</p>
                    <p className="mt-1 text-[10.5px] text-white/50 leading-tight">of {balance}</p>
                  </div>
                </div>
              </div>

              <div className="mt-4 grid grid-cols-2 gap-3">
                <button className="h-[48px] rounded-full bg-[#171a26] text-[16px] font-bold text-white">Withdraw</button>
                <button className="h-[48px] rounded-full bg-[#7c3aed] text-[16px] font-bold text-white">Deposit</button>
              </div>
            ...
            {/* Pick Types */}
            <h3 className="mt-7 text-[16px] font-medium text-white/55">PrizePicks Pick Types</h3>

            <button className="mt-3 flex h-[60px] w-full items-center gap-3 rounded-2xl border-[1.5px] border-white bg-transparent px-3">
              <span className="grid h-9 w-9 place-items-center rounded-lg bg-[#2a1758]">
                <svg viewBox="0 0 24 24" className="h-[19px] w-[19px] text-white" fill="none" stroke="currentColor" strokeWidth={2}>
              ...
              <span className="text-[18px] font-extrabold tracking-wide text-white">PLAYERS</span>
            </button>

            <button className="mt-3 flex h-[60px] w-full items-center rounded-2xl border border-white/[0.08] bg-[#0a0d18] px-3">
            ...
            {/* Useful links */}
            <h3 className="mt-8 text-[16px] font-medium text-white/55">Useful Links</h3>
            <div className="mt-1">
              {usefulLinks.map((l, i) =>
                typeof l === "string" ? (
                  <Row key={i} label={l} />
            ...
            <div className="mt-7 flex items-center justify-between">
              <button className="h-[48px] rounded-full bg-[#171a26] px-7 text-[16px] font-bold text-white">
                Log Out
              </button>
